...
class PurchaseReceipt(BuyingController):
	...
	def on_cancel(self):
		...
		#for val in frappe.db.sql("SELECT received_qty FROM `tabPurchase Order Item` WHERE parent='"+self.purchase_series+"'"):
		#	for item in self.get("items"):
		#		sub=val[0]-flt(item.received_qty)
		#		frappe.msgprint(str(val[0]))
				#frappe.db.sql("UPDATE `tabPurchase Order Item` SET `received_qty`='"+str(sub)+"' WHERE `parent`='"+self.purchase_series+"' AND `item_code`='" +item.item_code+"'")
...
def make_quality(source_name, target_doc=None):
	def update_item(obj, target, source_parent):
		if(obj.rejected_qty==0):
			target.qty = flt(obj.qty) - flt(obj.return_qty)
			target.amount = (flt(obj.qty) - flt(obj.return_qty)) * flt(obj.rate)
			target.base_amount = (flt(obj.qty) - flt(obj.return_qty)) * \
	...
	doc = get_mapped_doc("Purchase Receipt", source_name, {
		...
		"Purchase Receipt Item": {
			...
			"condition": lambda doc: abs(doc.received_qty) != abs(doc.return_qty)
		}
	}, target_doc)
